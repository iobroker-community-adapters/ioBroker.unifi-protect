...
class UnifiProtect extends utils.Adapter {
	...
	getLastMotionForCam(cameraId, lastMotitionTimestamp, onReady) {
		...
		const options = {
			hostname: this.config.protectip,
			port: this.config.protectport,
			path: (this.isUnifiOS ? this.paths.eventsUnifiOS : this.paths.events) + `?type=motion&camera=${cameraId}&end=${eventEnd}&start=${eventStart}`,
			method: "GET",
			rejectUnauthorized: false,
			resolveWithFullResponse: true,
			headers: {}
	...
	getMotionEvents(onReady) {
		...
		const options = {
			hostname: this.config.protectip,
			port: this.config.protectport,
			path: (this.isUnifiOS ? this.paths.eventsUnifiOS : this.paths.events) + `?end=${eventEnd}&start=${eventStart}&types=` + Object.keys(this.config.motionTypes).filter(key => this.config.motionTypes[key] === true).join("&types="),
			method: "GET",
			rejectUnauthorized: false,
			resolveWithFullResponse: true,
			headers: {}
