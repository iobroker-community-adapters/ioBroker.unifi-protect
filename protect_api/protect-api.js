...
class ProtectApi {
	...
	startWs() {
		...
		this.log.debug("Update listener: %s", this.paths.updates + "?" + params.toString());
		...
		try {
			...
			this.eventListener.on("error", (error) => {
				...
				if (error.message !== "WebSocket was closed before the connection was established") {
					this.log.error("%s: %s", this.config.protectip, error);
				}
			...
			this.eventListener.on("message", event => {
				...
				try {
					...
					if (error instanceof SyntaxError) {
						this.log.error("%s: Unable to process message from the realtime system events API: \"%s\". Error: %s.", this.config.protectip, event, error.message);
					} else {
						this.log.error("%s: Unknown error has occurred: %s.", this.config.protectip, error);
					}
			...
			this.log.info("%s: Connected to the UniFi realtime update events API.", this.config.protectip);
		} catch (error) {
			this.log.error("%s: Error connecting to the realtime update events API: %s", this.config.protectip, error);
		}
	}
