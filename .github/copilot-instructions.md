...
### UniFi Protect Adapter Specific Context

**Adapter Purpose**: This adapter connects ioBroker to Ubiquiti's UniFi Protect system, enabling monitoring and control of UniFi Protect cameras, sensors, and security devices.

**Target Systems**:
- UniFi Cloud Key Plus Gen2 (standard port: 7443)
- UniFi Dream Machine Pro (UDM Pro) (standard port: 443)
- UniFi Protect Controllers and Network Video Recorders

**Key Functionality**:
- Real-time camera monitoring and motion detection
- Smart detection events (person, vehicle, package, etc.)
- Manual and automated snapshot capture
- Video stream access and recording control
- Device status monitoring and configuration
- Motion event history and thumbnails
- WebSocket-based real-time updates

**Unique Requirements**:
- Requires authentication with UniFi Protect controllers
- Handles encrypted API communication
- Processes real-time WebSocket events for motion detection
- Manages binary image data (snapshots, thumbnails) as base64
- Supports both cloud-key and UDM Pro architectures
- Handles camera state management and device discovery
- Implements smart detection filtering and event processing
...
### UniFi Protect Adapter Specific Patterns

**API Communication**:
- Use `node-fetch` for HTTP requests to UniFi Protect controllers
- Implement proper authentication handling with login sessions
- Handle WebSocket connections for real-time events using the `ws` library
- Use `AbortController` for request timeout management

**Image Processing**:
- Handle base64 encoded images for thumbnails and snapshots
- Implement proper image resizing parameters in API calls
- Cache thumbnails and snapshots appropriately to avoid excessive API calls
- Support both automatic motion-triggered and manual snapshot capture

**Device Management**:
- Use proper device discovery patterns for cameras and sensors
- Implement state synchronization between UniFi Protect and ioBroker
- Handle device offline/online status changes gracefully
- Support both read-only and writable device properties

**Event Processing**:
- Process motion events, smart detection events, and device status changes
- Implement proper event filtering based on user configuration
- Handle event history and maintain motion event records
- Support real-time event streaming via WebSocket connections

**Error Handling**:
- Implement robust authentication error recovery (token refresh, re-login)
- Handle network connectivity issues gracefully
- Provide clear error messages for configuration issues
- Log appropriate detail levels for debugging API communication issues